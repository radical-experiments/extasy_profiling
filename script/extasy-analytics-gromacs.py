...
def create_workflow(Kconfig,args):
    ...
    while(cur_iter <  int(Kconfig.num_iterations) and cur_iter<start_iter+1):
        ...
        while(num_allocated_rep<num_replicas):
          ...
          sim_task.executable = [gromacs_path]
          ...
          (' %s/gromacs_example/inp_files/topol.top' % gromacs_example_path) +
              ' &> grompp.log')
          sim_task.pre_exec = [ 'module load gcc/8.1.1', grompp_bin ]
...
if __name__ == '__main__':
    ...
    try:
        ...
        appman = AppManager(hostname=hostname, port=port)#port=args.port)
